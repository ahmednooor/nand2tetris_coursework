...
class CompilationEngine:
    ...
    def printJsonToFile(self):
        self.json_safe_tree = copy.deepcopy(self.parse_tree)
        self.remove_recursive_pointers(self.json_safe_tree)
        with open(self.output_file_url+".json", "w") as f:
            f.write(json.dumps(self.json_safe_tree["children"][0], indent=2))
        # print(json.dumps(self.json_safe_tree, indent=4))

    def remove_recursive_pointers(self, node):
        del node["parent"]
        # if isinstance(node["parent"], dict):
        #     node["parent"] = node["parent"]["node_name"]
    
        if isinstance(node["children"], list):
            for child_node in node["children"]:
                self.remove_recursive_pointers(child_node)
